import { IsIn, IsEmail, IsDefined, IsOptional, ValidatorConstraint, ValidatorConstraintInterface, ValidationArguments, Validate, registerDecorator, ValidationOptions, isString, IsString, MinLength, MaxLength, Matches, IsNumber, Min, IsEnum, IsMimeType } from "class-validator";
import { MIN_INQUIRY_AMOUNT } from "../config/constants";
import { AvailableWithdrawCurrency, UserVerificationStatusEnum } from "../interfaces/requests/users";
...
export class RegisterRules {
  ...
  @MinLength(1)
  @MaxLength(10)
  @Matches(/^[A-Za-z0-9]+$/, {message: 'username cannot use special characters!'})
  username!: string;
}
...
export class UserWithdrawRules {
  @IsDefined()
  @IsString()
  @IsEnum(AvailableWithdrawCurrency, {message: 'currency must be one of the following values: `coin`, `revenue`'})
  currency !: string;
...
export class UpdateVerificationStatusRules {
  @IsDefined()
  @IsEnum(UserVerificationStatusEnum, { message: 'status should be between `pending`, `rejected`, or `verified`'})
  status!: string;
}
