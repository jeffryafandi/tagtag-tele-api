...
export class BankService extends BaseService {
    ...
    constructor(conn: Connection) {
        ...
        this.duitkuCustomerId   = config.DUITKU_CUSTOMER_ID;
    }
    public async fetchBankList(): Promise<Banks[]> {
        return this.dbConn.getRepository(Banks)
        .createQueryBuilder()
        .where('is_active = 1')
    ...
    public async fetchBankById(id: number): Promise<Banks|null> {
        return await this.dbConn.getRepository(Banks)
        .findOne({where: {id}});
    }

    public async fetchUserBankByAccountNumber(bank: Banks, accountNumber: string, user: Users | null = null): Promise<UserBanks|null> {
        let whereQuery: any = {
            bank_id         : bank.id,
        ...
        return await this.dbConn.getRepository(UserBanks)
    ...
    public async checkBankAccountFromDuitku(payload: CheckBankAccountRequest): Promise<any> {
        let bankCode = payload.bank_code;
        if (!bankCode && !payload.bank_id && !payload.operator_id) throw Error('bank_code or bank_ID is required!');

        if (payload.bank_id) {
            const bank = await this.fetchBankById(payload.bank_id);
            if (!bank) throw Error('Bank is not found!');
            const existed = await this.fetchUserBankByAccountNumber(bank, payload.account_number);
    ...
    public async storeUserBankAccount(user: Users, payload: CreateUserBankRequest): Promise<boolean> {
        const bank  = await this.fetchBankById(payload.bank_id);
        if (!bank) throw Error('Bank is not found!');

        const existed = await this.fetchUserBankByAccountNumber(bank, payload.account_number, user);
        ...
        await this.dbConn.getRepository(UserBanks).save({
            ...
            account_number  : inquiryResponse?.data?.bankAccount || payload.account_number
        });
