...
export class AuthService {
  ...
  public async isTokenValid(token: string): Promise<Users | null> {
    ...
    return await this.dbConn
      ...
      .leftJoinAndSelect(
        "user.bans",
        "bans",
        "bans.user_id = user.id AND bans.is_expired = 0"
      )
      .getOne();
  }
  ...
  public async validateRegistration(input: RegisterRequest): Promise<number> {
    ...
    const existingUser = await this.dbConn.getRepository(Users).findOne({
  ...
  public async CheckReferrerUsername(
    input: ReferrerUsernameCheck
  ): Promise<number> {
    const existingReferrerUsername = await this.dbConn
      .getRepository(Users)
  ...
  public async authCheckUsername(input: UsernameCheck): Promise<any> {
    const existingUsername = await this.dbConn
      .getRepository(Users)
      .findOne({ where: { username: input.username } });
  ...
  public async authCheckEmail(input: EmailCheck): Promise<any> {
    const existingEmail = await this.dbConn
      .getRepository(Users)
  ...
  public async validateGoogleIdRegistration(
    input: LoginWithGoogleIdRequest
  ): Promise<Users | null> {
    return await this.dbConn
      .getRepository(Users)
      .findOne({ where: { email: input.email } });
  ...
  public async validateRegistrationConfirmation(
    user: Users,
    confirmOtpToken: string
  ): Promise<number> {
  ...
  public async confirmRegistration(
    user: Users
  ): Promise<{ api_token: string; user_id: number }> {
  ...
  ): Promise<LoginWithUsernamePasswordResponse> {
  ...
  public async setForgotPasswordToken(
    user: Users,
  ...
  public async requestChangeEmailByUser(
    user: Users,
    email: string
  ): Promise<Users | undefined> {
    const totalExistings = await this.dbConn
      .getRepository(Users)
  ...
  public async confirmChangeEmailByUser(
    user: Users,
    otpToken: string
  ): Promise<boolean> {
  ...
  public async changePassword(
    ...
    if (
      user.password != null &&
      user.password != "" &&
      bcrypt.compareSync(input.old_password, user.password) === false
    ) {
      return -1;
  ...
  public async getUserFromToken(token: string): Promise<Users | null> {
    return await this.userService.getUserByApiToken(
      this.sanitizeRawToken(`${token}`)
    );
  }

  public async resetForgotPassword(
    payload: ForgotPasswordChangeRequest
  ): Promise<void | Error> {
    try {
      const user = await this.userService.getUserByResetPasswordToken(
  ...
  public async verifyAuthAddPin(user: Users, pinToken: string) {
    const userPinService = this.userService.userPinService;
    const userPin = await userPinService.findUnverifiedUserPinToken(
      user,
      pinToken
    );
  ...
  public async verifyForgotPinRequest(
    user: Users,
    schema: AuthForgotPinRequest
  ): Promise<boolean> {
    const userPinService = this.userService.userPinService;
    const userPins = await userPinService.fetchVerifiedUserPins(user);
    if (userPins.length == 0) throw Error("User doesn't have pin");
    const pinToken = Buffer.from(schema.request_pin_token, "base64").toString(
      "ascii"
    );
    const filtered = userPins.filter(
      (userPin) => userPin.request_pin_token == pinToken
    );
  ...
  public async checkIsPinAuthenticated(
    user: Users,
    pin: string
  ): Promise<{ is_valid: boolean; userPin?: UserPins }> {
    const userPinService = this.userService.userPinService;
    if (!user.user_pin_id) {
  ...
  public async validateGopayIdRegistration(
    input: LoginWithGopayIdRequest
  ): Promise<Users | null> {
    return await this.dbConn
      .getRepository(Users)
      .findOne({ where: { gopay_id: input.gopay_id } });
  }

  public async gopaySignin(
    gopay_id: string
  ): Promise<LoginWithUsernamePasswordResponse> {
    const user = await this.dbConn
      .getRepository(Users)
      .findOne({ where: { gopay_id: gopay_id } });
    let status = 1;
    if (!user) {
      status = -1;
    }
  ...
  public async getUserByTelegramId(telegramId: number): Promise<Users | null> {
    return await this.dbConn
      .getRepository(Users)
  ...
  public async returnTokenData(
    ...
    const user = await this.getUserFromToken(apiToken);
    if (!user) {
