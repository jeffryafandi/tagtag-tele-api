...
import {
  ...
  AFFILIATES_BONUS_COIN,
  AFFILIATES_BONUS_COUPON,
} from "../config/constants";
import { TransactionService } from "./transaction";
import {
  TRANSACTION_AVAILABLE_CODE,
  STAMINA_ENTRIES,
} from "../config/constants";
...
import {
  ...
  DuitkuProcessType,
  DuitkuTransferRequest,
} from "../interfaces/requests/duitku";
...
export class UserService {
  ...
  public async createUser(
    input: CreateUser,
    referred_user_id: number | undefined = undefined
  ): Promise<Users | undefined> {
    const userToSave: DeepPartial<Users> = {
    ...
    if (input.password && input.password.length > 0) {
      userToSave.password = bcrypt.hashSync(input.password, 10);
    }

    const user = await this.dbConn.getRepository(Users).save(userToSave);

    return user;
  }

  public async createUserGoogleId(
    input: CreateUserGoogleIdRequest
  ): Promise<Users> {
    const user = await this.dbConn.getRepository(Users).save({
      email: input.email,
  ...
  public async getUserByApiToken(apiToken: string): Promise<Users | null> {
    const user = await this.dbConn
      ...
      .leftJoinAndSelect(
        "users.bans",
        "bans",
        "bans.user_id = users.id AND bans.is_expired = 0"
      )
      .getOne();
  ...
  public async getAffiliateUser(user: Users): Promise<Affiliates | null> {
    return await this.dbConn
      ...
      .leftJoinAndSelect(
        "affiliateBenefit.affiliateBenefitTierings",
  ...
  public async getAffiliateUpgradeRequestUserByAffiliateId(
    affiliateId: number
  ): Promise<AffiliateUpgradeRequests | any> {
    return await this.dbConn
      .getRepository(AffiliateUpgradeRequests)
      .createQueryBuilder("affiliateUpgradeRequests")
      .where("affiliateUpgradeRequests.affiliate_id = :affiliateId", {
        affiliateId: affiliateId,
      })
      .andWhere("affiliateUpgradeRequests.status = :status", {
        status: "pending",
      })
      .getOne();
  }

  public async getUserByOtpToken(
    email: string,
    otpToken: string
  ): Promise<Users | null> {
    const user = await this.dbConn
  ...
  public async getUserAnalytic(input: FilterUserAnalytic) {
    ...
    const affiliateUsers = await this.affiliateService.getAllAffiliateUsers(
      input.start_date,
      input.end_date
    );
    ...
    const totalCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date
    );
    const totalPrizepoolCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date,
      "CR",
      [TransactionAvailableCodeEnum.PRIZEPOOL_REWARD]
    );
    const totalRaffleCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date,
      "CR",
      [TransactionAvailableCodeEnum.RAFFLE_REWARD]
    );
    const totalRefferalCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date,
      "CR",
      [TransactionAvailableCodeEnum.REFERRAL_CLAIM]
    );
    const totalMysteriBoxCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date,
      "CR",
      [TransactionAvailableCodeEnum.MYSTERY_BOX]
    );
    const totalAdminUpdateCoins =
      await this.transactionService.getTotalCurrency(
        TransactionDetailCurrencyEnum.COIN,
        input.start_date,
        input.end_date,
        "CR",
        [TransactionAvailableCodeEnum.ADMIN_USER_UPDATE]
      );
    const totalPlayMissionCoins =
      await this.transactionService.getTotalCurrency(
        TransactionDetailCurrencyEnum.COIN,
        input.start_date,
        input.end_date,
        "CR",
        [TransactionAvailableCodeEnum.PLAY_MISSIONS]
      );
    const totalDailyQuestCoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COIN,
      input.start_date,
      input.end_date,
      "CR",
      [TransactionAvailableCodeEnum.PLAY_DAILY_QUEST]
    );
    const totalPoins = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.ACTIVITY_POINT,
      input.start_date,
      input.end_date
    );
    const totalCoupons = await this.transactionService.getTotalCurrency(
      TransactionDetailCurrencyEnum.COUPON,
      input.start_date,
      input.end_date
    );
    const totalSpend = await this.transactionService.getTotalCurrency(
      [
        TransactionDetailCurrencyEnum.COIN,
        TransactionDetailCurrencyEnum.WITHDRAW_AMOUNT,
      ],
      input.start_date,
      input.end_date,
      "DB"
  ...
  public async getRegisteredUsers(startDate: string, endDate: string) {
    return await this.dbConn
      ...
      .where("users.created_at BETWEEN :startDate and :endDate", {
        startDate,
        endDate,
      })
      .getRawOne();
  ...
  public async mappedAllUsers(
    input: FilterUser | null
  ): Promise<[Users[], number]> {
    const users = this.dbConn
      .getRepository(Users)
      .createQueryBuilder("users")
      .where("users.is_confirmed IS TRUE")
      .andWhere("users.deleted_at IS NULL");
  ...
  public async getUserById(userId: number): Promise<Users | null> {
    return await this.dbConn.getRepository(Users).findOne({
      where: {
        id: userId,
      },
    });
  ...
  public async getUserByEmailAndForgotPasswordToken(
    email: string,
    token: string
  ): Promise<Users | null> {
    const user = await this.dbConn
  ...
  public async getUserByResetPasswordToken(
    token: string
  ): Promise<Users | null> {
    const user = await this.dbConn
      .getRepository(Users)
  ...
  public async update(
    ...
    schema: QueryDeepPartialEntity<Users>
  ): Promise<number> {
    ...
    const udpated = await this.dbConn
      .getRepository(Users)
      .createQueryBuilder()
      .update(Users)
      .set(schema)
  ...
  public mapUserActiveInventory(
    userInventories: UserGameInventories[]
  ): ActiveUserInventoryResponse[] {
    const activeData: ActiveUserInventoryResponse[] = userInventories.map(
      (userInventory) => {
        return {
  ...
  public async getMappedAuth(user: Users): Promise<object | undefined> {
    ...
    const notPlayed = [] as number[];

    for (const game of games) {
      const userScore = await this.gameService.fetchGameScoreByGameIdAndUserId(
        user.id,
        game.id
      );
      if (!userScore) {
    ...
    const userExtraLifeQuantity =
      await this.gameInventoryService.getUserExtraLife(user?.id);
    const extraLifeQuantity = userExtraLifeQuantity
      ? userExtraLifeQuantity.quantity
      : 0;
    ...
    const result = {
      ...
      has_pin:
        users?.userPin?.user_id === user.id && users.userPin?.is_verified
          ? true
          : false,
      ...
      affiliate_benefit: {
        ...
        percentage: users?.affiliate?.affiliateBenefit?.affiliateBenefitTierings
          .length
          ? users?.affiliate?.affiliateBenefit?.affiliateBenefitTierings[0]
              .value
          : 0,
      },
      vip: user?.vip,
    ...
    const activeInventories =
      await this.gameInventoryService.getActiveUserInventories(
        user,
        dayjs().format()
      );
    ...
    if (user.user_verification_id) {
      const verification = await this.fetchUserVerificationById(
  ...
  public async getAffiliateStatus(user: Users): Promise<object | void> {
    const affiliate = await this.getAffiliateUser(user);
    if (!affiliate) return;
    const latestRequest = await this.dbConn
      .getRepository(AffiliateUpgradeRequests)
      .findOne({
        join: {
    ...
    if (latestRequest) {
      switch (latestRequest.status) {
        case AvailableAffiliateStatus.rejected:
          upgradeAvailableAt = this.helperService.addDays(
    ...
    const result = {
      ...
      current_benefits: {
        ...
        percentage:
          affiliate.affiliateBenefit?.affiliateBenefitTierings[0].value,
      },
      previous_benefits: {
        id: latestRequest?.previous_benefit_id,
        name: latestRequest?.affiliateBenefit.name,
        percentage:
          latestRequest?.affiliateBenefit?.affiliateBenefitTierings[0].value,
      },
    };
    return result;
  }
  ...
  public async getAuthMyTeam(user: Users): Promise<object | undefined> {
    ...
    const users = await this.dbConn
      ...
      .createQueryBuilder("users")
      .where("users.deleted_at IS NULL")
      .andWhere("users.referred_user_id = :refferedUserId", {
        refferedUserId: user.id,
      })
      .getMany();
  ...
  public async getAuthTransactions(
    user: Users,
    code: TransactionAvailableCodeEnum | undefined = undefined
  ): Promise<object | undefined> {
    const mappedTransactions = [];
    const transactionQuery = this.dbConn
      .getRepository(Transactions)
      .createQueryBuilder("transactions")
  ...
  public async addAuthAffiliate(
    user: Users,
    input: AddAuthAffiliateRequest
  ): Promise<Affiliates> {
    input = {
      ...input,
      user_id: user.id,
      status: AvailableAffiliateStatus.approved,
    };
    const affiliate = await this.dbConn.getRepository(Affiliates).save(input);

    return affiliate;
  }

  public async addAffiliateSocials(
    input: AddAffiliateSocials
  ): Promise<AffiliateSocials | number | undefined> {
    const affiliateSocials = await this.dbConn
      .getRepository(AffiliateSocials)
      .save({
  ...
  public async addUserReferralPrize(
    input: AddUserReferralPrizeRequest
  ): Promise<UserReferralPrizes | number | undefined> {
    const refferralPrizes = await this.dbConn
      .getRepository(UserReferralPrizes)
      .save({
  ...
  public async substractUserLuckyWheelSpinEntries(
    users: Users,
    qtyToReduce: number = 1
  ) {
    return await this.update(users, {
      lucky_wheel_spin_entries:
        Number(users?.lucky_wheel_spin_entries) - qtyToReduce,
    });
  }

  public async storeUserWithdraw(
    schema: DeepPartial<UserWithdraws>
  ): Promise<UserWithdraws> {
    return await this.dbConn.getRepository(UserWithdraws).save(schema);
  }

  public async substractUserWithdrawAmount(
    user: Users,
    payload: UserWithdrawRequest
  ): Promise<boolean> {
    const userBank = await this.bankService.fetchUserBankById(
      payload.user_bank_id
    );
    ...
    const availableAmount =
      payload.currency == AvailableWithdrawCurrency.coin
        ? user.coins
    ...
    const inquiryRequest: DuitkuInquiryRequest = {
      amount:
        payload.amount - JSON.parse(`${adminFee.config_value}`).duitku.rtol_fee,
    ...
    const duitkuWithdrawResponse = await this.userDuitkuWithdraw(
      user,
      inquiryRequest,
      payload.currency
    );
    userWithdrawStatus = duitkuWithdrawResponse.success
  ...
  public async fetchAllUserDeviceTokens(): Promise<UserDevices[]> {
    const userDevices = await this.dbConn
  ...
  public async fetchUserDevicesByUserIDS(
    ids: string[]
  ): Promise<UserDevices[]> {
  ...
  public async storeUserDevice(
    schema: DeepPartial<UserDevices>
  ): Promise<UserDevices> {
    return await this.dbConn.getRepository(UserDevices).save(schema);
  }

  public async updateUserDeviceByID(
    id: number,
    payload: QueryDeepPartialEntity<UserDevices>
  ) {
    return await this.dbConn
      .getRepository(UserDevices)
      .createQueryBuilder()
  ...
  public async fetchUserExtPrizes(user: Users): Promise<object | undefined> {
    const prizes = await this.dbConn
      .getRepository(UserExtPrizes)
  ...
  public async getUserExtPrizebyExtPrizeId(
    user: Users,
    userExtPrizeId: number
  ): Promise<UserExtPrizes | null> {
  ...
  public async authPrizeClai(
    user: Users,
    userExtPrizeId: number
  ): Promise<void> {
    const userPrize = await this.getUserExtPrizebyExtPrizeId(
      user,
      userExtPrizeId
    );
    if (!userPrize) return;
  ...
  public async authPrizeClaim(
    user: Users,
    payload: UserClaimPrize,
    userExtPrizeId: number
  ...
  public async userDuitkuWithdraw(
    user: Users,
    payload: DuitkuInquiryRequest,
    currency: AvailableWithdrawCurrency
  ): Promise<{ success: boolean; data: any }> {
    const inquiryResponse = await this.bankService.duitkuService.inquiry(
      payload
    );
    ...
    if (inquiryResponse.success) {
      ...
      transferResponse = await this.bankService.duitkuService.transfer(
        transferRequest
      );
      ...
      if (transferResponse.success) {
        if (currency == AvailableWithdrawCurrency.coin) {
          ...
          await this.update(user, {
            coins:
              Number(user.coins) - (payload.real_withdrawn || payload.amount),
          });
        } else {
          await this.update(user, {
            withdrawable_amount:
              Number(user.withdrawable_amount) -
              (payload.real_withdrawn || payload.amount),
          });
        }
    ...
    if (success) {
      ...
      details.push({
        type: "DB",
        currency:
          currency == AvailableWithdrawCurrency.coin
            ? TransactionDetailCurrencyEnum.COIN
            : TransactionDetailCurrencyEnum.WITHDRAW_AMOUNT,
  ...
  public async userAwdWithdraw(
    user: Users,
    payload: AwdPrepaidRequest,
    operator: Operators,
    currency: AvailableWithdrawCurrency
  ): Promise<{ success: boolean; data: any }> {
    ...
    if (prepaidResponse.success) {
      if (currency == AvailableWithdrawCurrency.coin) {
        ...
        await this.update(user, {
          withdrawable_amount:
            Number(user.withdrawable_amount) - operator.price,
        });
      }
  ...
  public async userWithdrawToEWallet(
    user: Users,
    payload: UserWithdrawEWalletRequest
  ): Promise<boolean> {
    let userWithdrawStatus = UserWithdrawStatus.failed;
    ...
    if (
      !operator ||
      operator.price > availableAmount ||
      operator.vendor !== activeVendor
    )
      return false;
    ...
    switch (activeVendor) {
      case "awd":
        ...
        const awdWithdrawResponse = await this.userAwdWithdraw(
          user,
          prepaidRequest,
          operator,
          payload.currency
        ...
        const awdUserWithdraw = await this.storeUserWithdraw({
          ...
          status: awdWithdrawResponse.success
            ? UserWithdrawStatus.success
            : UserWithdrawStatus.failed,
        });
        ...
        await this.awdService.storeAwdLogs(
          {
            ...
            json_response: JSON.stringify(
              awdWithdrawResponse.data?.prepaidResponse?.data
            ),
          },
          user
        );
        userReceive = operator.denom;
        userWithdrawStatus = awdWithdrawResponse.success
          ? UserWithdrawStatus.success
          : UserWithdrawStatus.failed;
        break;
      ...
      default: // duitku
        ...
        const inquiryRequest: DuitkuInquiryRequest = {
          amount:
            operator.price -
            JSON.parse(`${adminFee.config_value}`).duitku.e_wallet_fee,
    ...
    let notifCode = NOTIF_CODE.WITHDRAW_COIN_FAIL;
    const notificationParams: any = { nominal: userReceive };
  ...
  public async fetchUserVerificationById(
    id: number
  ): Promise<UserVerifications | null> {
    return await this.dbConn.getRepository(UserVerifications).findOne({
      where: { id },
      join: {
  ...
  public async storeUserVerifications(
    user: Users,
    data: { image: string }
  ): Promise<boolean> {
    if (user.user_verification_id) {
  ...
  public async updateUserVerification(data: {
    ...
    const updated = await this.dbConn
      .getRepository(UserVerifications)
      .update(verification.id, { status: data.status });
    ...
    if (isUpdated) {
      ...
      await this.update(verification.user_id, {
  ...
  public async updateUserById(data: {
    user: Users;
    userId: number;
    input: UpdateUser;
  }): Promise<boolean> {
    const updateSchema = {
      ...
      activity_points:
        data.user.activity_points + (data.input.add_activity_point ?? 0),
    };
    ...
    if (data.input.add_coins) {
      ...
      await this.transactionService.storeUserTransaction(
        data.user,
        transactionPayload
      );
    }
  ...
  public async getAllUsersExceptIDs(
    userIds: Array<number>,
    filter: { username?: string } | undefined = undefined
  ): Promise<Array<Users>> {
    const query = this.dbConn
      .getRepository(Users)
      .createQueryBuilder()
  ...
  public async banUserByIds(payload: BanUnBanUsersRequest) {
    const bulkSchema: QueryDeepPartialEntity<UserBans>[] = payload.user_ids.map(
      (id) => ({
        user_id: id,
  ...
  public async updateBan(
    userBanId: number,
    schema: QueryDeepPartialEntity<UserBans>
  ) {
    await this.dbConn
      .getRepository(UserBans)
      .createQueryBuilder()
  ...
  public async findUserBans(
    filter: { canExpired: boolean } = { canExpired: false }
  ): Promise<UserBans[]> {
    const query = this.dbConn
      .getRepository(UserBans)
      .createQueryBuilder()
  ...
  public async userWithdrawDetail(startDate: string, endDate: string) {
    const mapped = [];
    const duitkuLogs = await this.duitkuService.getUsersDuitkuLogs(
      startDate,
      endDate
    );
  ...
  public async checkForUpdate(currentVersion: string): Promise<boolean> {
    const minVersionConfig = await this.appConfigService.getConfigByKey(
      APP_CONFIG_KEY.minVersion
    );
