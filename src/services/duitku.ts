...
import {
  DuitkuPossibleTransferResponse,
  DuitkuResponseCodes,
} from "../interfaces/responses/duitku";
...
export class DuitkuService {
  ...
  constructor(Connection: Connection) {
    ...
    this.url = process.env.DUITKU_URL;
    this.userId = process.env.DUITKU_USER_ID;
    this.customerId = process.env.DUITKU_CUSTOMER_ID;
    this.email = process.env.DUITKU_EMAIL;
    this.secretKey = process.env.DUITKU_SECRET_KEY;
    this.inquiryEndpoint = process.env.DUITKU_INQUIRY_ENDPOINT;
    this.transferEndpoint = process.env.DUITKU_TRANSFER_ENDPOINT;
  }
  ...
  public async callAPI(
    ...
    [
      success: boolean,
      message: string,
      data: DuitkuPossibleTransferResponse | undefined
    ]
  > {
    ...
    try {
      const response =
        await this.helperService.post<DuitkuPossibleTransferResponse>(
          url + endpoint,
          payload,
          header
        );
  ...
  public async inquiry(
    ...
    const [success, message, data] = await this.callAPI(
      payload,
      `${this.inquiryEndpoint}`
    );
  ...
  public async storeDuitkuLogs(
    schema: DeepPartial<DuitkuLogs>
  ): Promise<DuitkuLogs> {
  ...
  public async getUsersDuitkuLogs(startDate: string, endDate: string) {
    ...
    const logs = await this.dbConn
      .getRepository(DuitkuLogs)
      ...
      .where("duitkuLogs.created_at BETWEEN :startDate and :endDate", {
        startDate,
        endDate,
      })
      .getMany();
    ...
    for (const log of logs) {
      ...
      const bankId = bankList
        .filter((bank_id) => bank_id.bank_code == bankCode)
        .map((b) => b.id)
        .toString();
      const bankName = bankList
        .filter((bank_name) => bank_name.bank_code == bankCode)
        .map((b) => b.name)
        .toString();
