...
export class Database {
    ...
    public async injectOptions(connection: DataSource, connectionOptions: DataSourceOptions): Promise<DataSource>{
        // @ts-ignore
    ...
    public async getDataSource(): Promise<DataSource> {
        ...
        if (datasource.isInitialized) {
            ds = await this.injectOptions(this.connectionManager.get(CONNECTION_NAME), dsOptions);
        } else {
        ...
        try {
            ...
            if (this.connectionManager.has(CONNECTION_NAME)) {
                ...
                if (!connection.isConnected) {
                    console.log("Existing connection found but not connected. Removing and recreating...");

                    // Remove from connectionManager safely
                    const index = this.connectionManager.connections.findIndex(c => c.name === CONNECTION_NAME);
                    if (index !== -1) {
                        (this.connectionManager.connections as any).splice(index, 1);
                    }
